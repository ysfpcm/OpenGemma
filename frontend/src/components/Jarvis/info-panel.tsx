...
function MetricTile({ m }: { m: Metric }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-foreground/5 p-3">
      <div className="font-mono text-[8px] tracking-widest text-muted-foreground">{m.label}</div>
      <div className="mt-0.5 flex items-baseline gap-1">
        <span className={cn("font-mono text-xs text-glow", m.status ? STATUS_COLOR[m.status] : "text-foreground")}>
          {m.value}
        </span>
        {m.trend && <span className="font-mono text-[8px] text-muted-foreground">{m.trend}</span>}
      </div>
    </div>
  )
}
...
export function InfoPanel({ messages, onSendMessage, busy }: { messages: ChatMessage[], onSendMessage: (text: string) => void, busy?: boolean }) {
  ...
  return (
    <div className="flex h-full flex-col gap-4">
        ...
        {messages.map((m) => (
          <div key={m.id} className={cn("flex w-full", m.role === "user" ? "justify-end" : "justify-start")}>
            <div className={cn(
              ...
              {m.role === "assistant" ? (
                <div className="prose prose-invert max-w-none font-sans text-base md:text-lg leading-relaxed">
                  <ReactMarkdown
                ...
                <div className="font-sans text-base">{m.content}</div>
              )}
            </div>
          </div>
        ))}
      ...
      {/* Chat Input */}
      <div className="mt-auto shrink-0 mb-3">
      ...
      <div className="flex flex-col sm:flex-row gap-3 shrink-0">
        ...
        <div className="flex-1 rounded-lg border border-border bg-foreground/[0.03] p-2 shrink-0 overflow-hidden">
          <div className="mb-1.5 flex items-center gap-1.5 font-mono text-[8px] tracking-widest text-muted-foreground">
            <Terminal className="h-2.5 w-2.5 text-primary" />
            <span>EXECUTION LOG</span>
          </div>
          <ul className="flex flex-col gap-0.5 max-h-16 overflow-y-auto font-mono text-[9px]">
            {executionLogs.length === 0 ? (
              ...
              executionLogs.map((log, i) => (
                ...
                >
                  ...
                  <span className="ml-auto uppercase text-[8px] tracking-wider">
        ...
        {metrics.length > 0 && (
          <div className="grid grid-cols-2 gap-1.5 shrink-0 min-w-[200px]">
            {metrics.map((m) => (
