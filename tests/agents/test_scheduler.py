...
class TestSchedulerBasic:
    def test_cron_fallback_respects_daily_schedule(self):
        from datetime import datetime

        from openjarvis.agents.scheduler import _next_cron_fire_without_croniter

        base = datetime(2026, 8, 10, 18, 54).timestamp()
        next_fire = datetime.fromtimestamp(
            _next_cron_fire_without_croniter("0 5 * * *", base)
        )

        assert next_fire.strftime("%Y-%m-%d %H:%M") == "2026-08-11 05:00"

    def test_cron_fallback_respects_step_and_hour_range(self):
        from datetime import datetime

        from openjarvis.agents.scheduler import _next_cron_fire_without_croniter

        base = datetime(2026, 8, 10, 5, 1).timestamp()
        next_fire = datetime.fromtimestamp(
            _next_cron_fire_without_croniter("*/5 5-6 * * *", base)
        )

        assert next_fire.strftime("%Y-%m-%d %H:%M") == "2026-08-10 05:05"
