...
class TestAskDirectEngineMode:
    def test_direct_clock_lookup_uses_authoritative_local_tool(self):
        engine = _FakeEngine({"content": "Friday, July 4, 2026"})
        system = _FakeSystem(engine=engine)
        orchestrator = QueryOrchestrator(system)

        result = orchestrator.ask("What day is it today?", context=False)

        assert result["content"].startswith("Today is ")
        assert "July 4, 2026" not in result["content"]
        assert result["metadata"]["routing_reason"] == "authoritative_clock"
        assert result["tool_results"][0]["tool_name"] == "clock"
