...
class _LightweightSystem:
    ...
    def __init__(
        ...
        self.model = model
        self.config = config
        # Background agent ticks use this lightweight facade instead of the
        # full JarvisSystem. Preserve the live channel bridge so channel_send
        # can reach configured providers such as SendBlue.
...
def _make_lightweight_system(
    ...
    try:
        ...
        return _LightweightSystem(plain_engine, model, cfg, channel_backend)
    except Exception:
        pass
    return _LightweightSystem(engine, model, config, channel_backend)
...
def _resolve_tool_specs(
    ...
    try:
        ...
        importlib.import_module("openjarvis.connectors")  # Ensure registration
    except ImportError:
        pass
...
def create_agent_manager_router(
    ...
    def list_agent_status(request: Request):
        """Return operator-facing run, schedule, tool, and delivery status."""
        ...
        for agent in manager.list_agents():
            ...
            if trace_store is not None:
                try:
                    traces = trace_store.list_traces(agent=agent["id"], limit=1)
                    if traces:
                        trace = traces[0]
                        latest_trace = {
                    ...
                    logger.debug(
                        "Unable to load latest trace for agent %s",
                        agent["id"],
                        exc_info=True,
                    )
            schedule = (
                scheduler.get_agent_schedule(agent["id"])
                if scheduler is not None
                else {
                    "schedule_type": agent.get("config", {}).get(
                        "schedule_type", "manual"
                    ),
                    "schedule_value": agent.get("config", {}).get(
                        "schedule_value", ""
                    ),
                    "next_run_at": None,
                }
            ...
            statuses.append(
                build_agent_run_status(
                    agent,
                    messages=manager.list_messages(agent["id"], limit=50),
                    schedule=schedule,
    ...
    async def run_agent(agent_id: str, request: Request):
        ...
        server_channel = getattr(request.app.state, "channel_bridge", None)
    ...
    async def send_message(agent_id: str, req: SendMessageRequest, request: Request):
        ...
        if not req.stream and req.mode == "immediate":
            ...
            _srv_channel = getattr(request.app.state, "channel_bridge", None)
                ...
                try:
                    ...
                    executor = AgentExecutor(
                        ...
                        context_store=getattr(
                            request.app.state, "context_store", None
                        ),
                    )
                    system = _make_lightweight_system(
                        _srv_engine,
