...
def create_app(
    ...
    *,
    ...
    app.state.home_assistant_bridge = None
    app.state.context_sync_service = None

    # Local context memory exists independently of any one connector. Home
    # Assistant can push events into it, while lightweight polling collectors
    # keep external facts (such as traffic) warm without invoking an agent.
    ha_token = (
    ...
    try:
        ...
        context_store = ContextStore(
            os.environ.get("OPENJARVIS_CONTEXT_DB", "").strip() or None
        )
        app.state.context_store = context_store
        # The long-lived scheduler owns an AgentExecutor created before the
        # FastAPI context service. Attach the shared store once it exists.
        scheduled_executor = getattr(agent_scheduler, "_executor", None)
        if scheduled_executor is not None:
            scheduled_executor.set_context_store(context_store)

        collectors = []
        traffic_snapshot_path = os.environ.get(
            "OPHANIM_TRAFFIC_SNAPSHOT_PATH", ""
        ).strip()
        if not traffic_snapshot_path:
            traffic_snapshot_path = str(
                pathlib.Path.home()
                / ".openjarvis"
                / "shared"
                / "commute-traffic.json"
            )
        if pathlib.Path(traffic_snapshot_path).is_file():
            collectors.append(
                TrafficSnapshotFileCollector(
                    traffic_snapshot_path,
                    interval_seconds=float(
                        os.environ.get("OPHANIM_TRAFFIC_FILE_INTERVAL_SECONDS", "30")
                    ),
                )
            )
        traffic_origin = os.environ.get("OPHANIM_TRAFFIC_ORIGIN", "").strip()
        traffic_destination = os.environ.get(
            "OPHANIM_TRAFFIC_DESTINATION", ""
        ).strip()
        if traffic_origin and traffic_destination:
            collectors.append(
                TrafficContextCollector(
                    origin=traffic_origin,
                    destination=traffic_destination,
                    route_name=os.environ.get(
                        "OPHANIM_TRAFFIC_ROUTE_NAME", "commute"
                    ),
                    interval_seconds=float(
                        os.environ.get("OPHANIM_TRAFFIC_INTERVAL_SECONDS", "300")
                    ),
                )
            )
        sync_service = ContextSyncService(context_store, collectors)
        app.state.context_sync_service = sync_service
        ...
        if ha_token:
            ha_bridge = HomeAssistantBridge(
                ...
                token=ha_token,
                bus=bus,
                snapshot_dir=os.environ.get("HA_SNAPSHOT_DIR"),
            )
            app.state.home_assistant_bridge = ha_bridge

        @app.on_event("startup")
        async def _start_context_services() -> None:
            service = getattr(app.state, "context_sync_service", None)
        ...
        async def _shutdown_context_services() -> None:
            bridge = getattr(app.state, "home_assistant_bridge", None)
            if bridge is not None:
                await bridge.stop()
            service = getattr(app.state, "context_sync_service", None)
        ...
        if context_store is not None:
            context_store.close()
        # Context collectors are optional; the API must still start. Avoid
        # exception text because dependencies sometimes echo credential URLs.
        logger.debug("Context service init skipped: %s", type(exc).__name__)
