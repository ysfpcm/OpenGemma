...
class AgentExecutor:
    ...
    def set_context_store(self, context_store: Any) -> None:
        """Attach the shared live-context store used by every managed tick."""
        self._context_store = context_store
    ...
    def _inject_tool_deps(self, tool: Any) -> None:
        ...
        elif name.startswith("channel_") or name == "commute_readiness":
    ...
    def execute_tick(
        ...
        # Persist a compact tool ledger with the response so the Operations
        # view can distinguish "the agent ran" from "the requested action
        # was delivered" after a page reload.
        tool_calls: list[dict[str, Any]] = []

        def _on_tool_start(event: Any) -> None:
            if event.data.get("agent") == agent_id:
                arguments = event.data.get("arguments") or event.data.get("args")
        ...
        def _on_tool_end(event: Any) -> None:
            ...
            for call in reversed(tool_calls):
                if "success" not in call:
                    call["result"] = str(event.data.get("result", ""))[:4096]
                    call["success"] = bool(event.data.get("success", False))
                    call["latency"] = event.data.get("latency", 0)
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        # Live context is a current-state projection (traffic, Home Assistant,
        # website monitors, and future collectors), distinct from semantic
        # document memory.  Read it before every managed tick so both scheduled
        # and user-triggered agents operate on the same fresh snapshot.
        context_store = self._context_store or getattr(
            self._system, "context_store", None
        )
        owns_context_store = False
        try:
            from openjarvis.context import ContextBuilder, ContextRequest, ContextStore

            if context_store is None:
                # CLI-created executors do not have an application lifecycle
                # to inject dependencies. They still use the canonical local
                # context database, and close their short-lived connection.
                context_store = ContextStore()
                owns_context_store = True
            if context_store is not None:
                snapshot = ContextBuilder(context_store).build(
                    ContextRequest(recent_event_limit=20)
                )
                live_context = snapshot.to_prompt_text()
                input_text = (
                    "Current context database snapshot:\n"
                    f"{live_context}\n\n{input_text}"
                )
                agent_ctx.metadata["context_snapshot"] = live_context
                agent_ctx.metadata["context_generated_at"] = snapshot.generated_at
        except Exception:
            logger.warning(
                "Agent %s: live context lookup failed",
                agent["name"],
                exc_info=True,
            )
        finally:
            if owns_context_store and context_store is not None:
                context_store.close()
